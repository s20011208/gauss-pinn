...
def train(rank, world_size, config):
    ...
    def train_with_lbfgs(f, cfg, dataset, heat: HeatEquation, initial_lr=1.0, max_iter=20000):
        optimizer = torch.optim.LBFGS(f.parameters(), lr=initial_lr, max_iter=max_iter)
    ...
    for i in range(cfg.train.iteration):
        ...
        if rank==0 and (i+1)%cfg.train.iteration==0:
            ...
            train_with_lbfgs(f, cfg, dataset, heat, initial_lr=1.0, max_iter=20000)
            ...
            observe_t = np.array([])
            for i in range(100):
                observe_x = np.concatenate((observe_x, np.linspace(0.0,1.0,100)))
...
def generate_test_set(cfg, heat: HeatEquation, rank):
    x = torch.rand((cfg.test.total_size, heat.xdim), device=rank)
    test_X = torch.concat(
        [x, 
